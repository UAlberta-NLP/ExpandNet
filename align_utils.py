  ...
  while unfinished:
    ...
    for i in unaligned_source_indices + consecutive_subsequences(unaligned_source_indices, max_subseq_l):
        ...
        for j in unaligned_target_indices + consecutive_subsequences(unaligned_target_indices, max_subseq_l):
            ...
            # Check for synonyms
            are_syn = alignobj.are_synonyms_by_dictionary(word_one, word_two, lemma_one, lemma_two, tl, sl)
...
def babelnet_pass(sl, tl, src_wds, tgt_wds, src_lms, tgt_lms, align_ans, unaligned_source_indices, unaligned_target_indices, strict_lemma, alignobj, sjc, tjc, strict_intersect):
  unfinished = True
  ...
  while unfinished:
    ...
    for i in unaligned_source_indices.copy():
        if isinstance(i, int):
            ...
            word_one = sjc.join([src_wds[el] for el in i])
            lemma_one = sjc.join([src_lms[el] for el in i])
    ...
    for j in unaligned_target_indices.copy():
        ...
        if isinstance(j, int):
                ...
                word_two = tjc.join([tgt_wds[el] for el in j])
                lemma_two = tjc.join([tgt_lms[el] for el in j])
...
def intersectmwe_pass(src_lang, tar_lang, src_wds, tgt_wds, src_lems, tgt_lems, align_ans, unaligned_source_indices, unaligned_target_indices, strict_lemma, aligner, s_j_c, t_j_c, max_s_l):
    babelmwe_pass(src_lang, tar_lang, src_wds, tgt_wds, src_lems, tgt_lems, align_ans, unaligned_source_indices, unaligned_target_indices, strict_lemma, aligner, s_j_c, t_j_c, max_s_l, False, False)
    babelmwe_pass(src_lang, tar_lang, src_wds, tgt_wds, src_lems, tgt_lems, align_ans, unaligned_source_indices, unaligned_target_indices, strict_lemma, aligner, s_j_c, t_j_c, max_s_l, False, True)
...
def cognates(word_one, word_two):
    return False
    if word_one[:3] == word_two[:3]:
        return True
    return False

def every_word_cognates(words_one, words_two):
